...
export default function UploadPermit ({navigation}){
	...
	const handleUploadPhoto = () => {
		AsyncStorage.getItem('user_id').then( (res) => {
			uploadAttachments({ 'user_id' : res ,'permit':image64}).then((res) => {
				console.log(res)
			} , (err) => {
				console.log(err)
			})
			navigation.navigate('UploadPoliceChalan');
		})
	};
		...
		<ScrollView vertical={true}>
			<View style={{ flex: 1, width: '100%',height: (deviceHeight) }}>
				<View style={{flex: 2,width : '100%',justifyContent: 'center',padding: 20}}>
					<TouchableHighlight  onPress={() => { handleChoosePhoto('permit') }} style={{borderRadius : 13 , borderWidth: 2 , borderColor: 'lightgrey', backgroundColor: '#e8e8e8',height: 200 ,marginTop: 20,minHeight: 200, width: '100%',justifyContent: 'center', alignItems: 'center'}} >
						{(permit != null)?
							<View>
								<Image source={{ uri: permit.uri }} style={{ width: 150, height: 150,borderRadius: 20 }} />
							</View>
						:
							<View>
