...
export default function UploadFourSidePhoto({navigation}){
	...
	// let base64Data = [];
	const [ActvInd, setActivateInd] = React.useState(false);
	const [convertedToBase64, setconvertedToBase64] = React.useState(false);
	...
	const handleUploadPhoto = () => {
		AsyncStorage.getItem('user_id').then( (res) => {
			uploadAttachments({ 'user_id' : res ,'rc':image64}).then((res) => {
				console.log(res)
			} , (err) => {
				console.log(err)
			})
