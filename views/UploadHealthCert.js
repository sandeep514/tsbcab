...
export default function UploadHealthCert({navigation}){
	...
	const handleUploadPhoto = () => {
		AsyncStorage.getItem('user_id').then( (res) => {
			uploadAttachments({ 'user_id' : res ,'healthCert':image64}).then((res) => {
				console.log(res)
			} , (err) => {
				console.log(err)
			})
			navigation.navigate('UploadMedicalCertif');
		})
	};
			...
			<View style={{ flex: 1, width: '100%',height: (deviceHeight) }}>
				<View style={{width : '100%',justifyContent: 'center'}}>
					<TouchableHighlight  onPress={() => { handleChoosePhoto('healthCert') }} style={{borderRadius : 13 , borderWidth: 2 , borderColor: 'lightgrey', backgroundColor: '#e8e8e8',height: 200 ,marginTop: 20,minHeight: 200, width: '100%',justifyContent: 'center', alignItems: 'center'}} >
						{(healthCert != null)?
							<View>
								<Image source={{ uri: healthCert.uri }} style={{ width: 150, height: 150,borderRadius: 20 }} />
							</View>
						:
							<View>
