...
export default function UploadDrivingLicence({navigation}){
	...
	const handleUploadPhoto = () => {
		AsyncStorage.getItem('user_id').then( (res) => {
			uploadAttachments({ 'user_id' : res ,'drivingLicence':image64}).then((res) => {
				console.log(res)
			} , (err) => {
				console.log(err)
			})
			// navigation.navigate('UploadFourSidePhoto');
			navigation.navigate('UploadHealthCert');
		})
	};
		...
		<ScrollView vertical={true}>
			<View style={{ flex: 1,justifyContent: 'center', width: '100%',height: deviceHeight}}>
				<View style={{flex: 2,width : '100%',justifyContent: 'center',padding: 20}}>
					<TouchableHighlight  onPress={() => { handleChoosePhoto('drivingLicence') }} style={{borderRadius : 13 , borderWidth: 2 , borderColor: 'lightgrey', backgroundColor: '#e8e8e8',height: 200 ,marginTop: 20,minHeight: 200, width: '100%',justifyContent: 'center', alignItems: 'center'}} >
						{(drivingLicence != null)?
							<View>
								<Image source={{ uri: drivingLicence.uri }} style={{ width: 150, height: 150,borderRadius: 20 }} />
							</View>
						:
							<View>
